<template>
  <el-form :model="formItem"
    ...
    <el-form-item :label="$t('tobacco.tstorage.gradeContrast.ogradeName')">
      <type-grade-select :typeId.sync="formItem.typeId"
                         :typeName.sync="formItem.typeName"
                         :gradeId.sync="formItem.ogradeId"
                         :gradeName.sync="formItem.ogradeName"
    ...
    <el-form-item :label="$t('tobacco.tstorage.gradeContrast.cgradeName')">
      <type-grade-select :typeId.sync="formItem.typeId"
                         :typeName.sync="formItem.typeName"
                         :gradeId.sync="formItem.cgradeId"
                         :gradeName.sync="formItem.cgradeName"
                         :gradeCode.sync="formItem.cgradeCode"
                         style="width: 100%;">
      </type-grade-select>
    </el-form-item>
    <el-form-item :label="$t('tobacco.tstorage.gradeContrast.gap')">
      <el-input-number v-model="formItem.gap"
                       style="width: 100%" />
    </el-form-item>
  ...
  methods: {
    ...
    onSubmitClick(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          Promise.all([
            gradeContrastApi.updateGradeContrast(
              this.formItem.id,
              this.formItem
            )
          ])
