  ...
  <div class='mainPanel'>
      ...
      <div class='queryCriteria'>
        <el-form inline>
          <el-form-item :label="$t('tobacco.tstorage.operationFromTo.organizationId')">
            <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="searchData.organizationId"
                      @keyup.enter.native="onSearchButtonClick"/>
          </el-form-item>
          <el-form-item :label="$t('tobacco.tstorage.operationFromTo.warehouseName')">
            <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="searchData.warehouseId"
                      @keyup.enter.native="onSearchButtonClick"/>
          </el-form-item>
    ...
    <main class='contentPanel'>
      <el-table
        ...
        :data="formData.operationFromToList"
        style="width: 100%"
        :row-class-name="tableRowClassName">
        <el-table-column type="index">
        </el-table-column>
        <el-table-column prop="organization.organizationId" :label="this.$t('tobacco.tstorage.operationFromTo.organizationId')"/>
        <el-table-column prop="organization.organizationName" :label="this.$t('tobacco.tstorage.operationFromTo.organizationName')"/>
        <el-table-column prop="warehouseId" :label="this.$t('tobacco.tstorage.operationFromTo.warehouseId')"/>
        <el-table-column prop="warehouseName" :label="this.$t('tobacco.tstorage.operationFromTo.warehouseName')"/>
        <el-table-column prop="threeOrganizationId"
                         :label="this.$t('tobacco.tstorage.operationFromTo.threeOrganizationId')"/>
        <el-table-column prop="threeOrganizationName"
                         :label="this.$t('tobacco.tstorage.operationFromTo.threeOrganizationName')"/>
        <el-table-column prop="threeWarehouseId" :label="this.$t('tobacco.tstorage.operationFromTo.threeWarehouseId')"/>
        <el-table-column prop="threeWarehouseName"
                         :label="this.$t('tobacco.tstorage.operationFromTo.threeWarehouseName')"/>
        <el-table-column prop="desc" :label="this.$t('tobacco.tstorage.operationFromTo.desc')"/>
        <el-table-column
          fixed="right"
...
<script>
  const AddForm = () => import('./operationFromToAdd.vue');
  const EditForm = () => import('./operationFromToEdit.vue');
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([operationFromToApi.deleteOperationFromTo(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([operationFromToApi.getAll({
          ...
          sort: 'id,ASC',
          contains: 'organizationId,organizationName,warehouseId,:{keyword}:true'.format({keyword: this.formData.pagination.keyword}),
          search: 'organizationId:EQ:{organizationId};organizationName:EQ:{organizationName};warehouseId:EQ:{warehouseId};'.format({
            organizationId: this.searchData.organizationId,
            organizationName: this.searchData.organizationName,
            warehouseId: this.searchData.warehouseId,
          })
        })])
          .then(([response]) => {
            this.formData.operationFromToList = response.content;
