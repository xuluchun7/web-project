          ...
          <el-row>
            <template v-if='expand'>
              <el-form-item :label="$t('tobacco.tstorage.gradeContrast.ogradeName')">
                <type-grade-select :typeId.sync="searchData.typeId"
                                   :typeName.sync="searchData.typeName"
                                   :gradeId.sync="searchData.ogradeId"
                                   :gradeName.sync="searchData.ogradeName"
              ...
              <el-form-item :label="$t('tobacco.tstorage.gradeContrast.cgradeName')">
                <type-grade-select :typeId.sync="searchData.typeId"
                                   :typeName.sync="searchData.typeName"
                                   :gradeId.sync="searchData.cgradeId"
                                   :gradeName.sync="searchData.cgradeName"
    ...
    <main class='contentPanel'>
      <el-table highlight-current-row
        ...
        <el-table-column prop="typeName"
                         ...
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="ogradeName"
                         :label="this.$t('tobacco.tstorage.gradeContrast.ogradeName')" />
        <el-table-column prop="cgradeName"
                         :label="this.$t('tobacco.tstorage.gradeContrast.cgradeName')" />
        <el-table-column prop="gap"
                         :label="this.$t('tobacco.tstorage.gradeContrast.gap')" />
        <el-table-column prop="desc"
                         :label="this.$t('tobacco.tstorage.gradeContrast.desc')" />
        <!--
                <el-table-column
                  fixed="right"
...
export default {
  ...
  methods: {
    ...
    deleteButtonClick() {
      ...
      Promise.all([
        gradeContrastApi.deleteGradeContrast(this.formData.selectRow.id)
      ])
        .then(([response]) => {
          this.$message({
            type: "info",
    ...
    onSearchButtonClick() {
      Promise.all([
        gradeContrastApi.getAll({
          size: this.formData.pagination.pageSize,
          page: this.formData.pagination.currentPage - 1,
          sort: "id,ASC",
          contains: "ogradeName,cgradeName,gap,desc,:{keyword}:true".format({
