    ...
    <main class='contentPanel'>
      <el-table highlight-current-row
        ...
        <el-table-column prop="annual"
                         :label="this.$t('tobacco.tstorage.smokepoint.annual')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="serial"
                         :label="this.$t('tobacco.tstorage.smokepoint.serial')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="title"
                         :label="this.$t('tobacco.tstorage.smokepoint.title')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="author"
                         :label="this.$t('tobacco.tstorage.smokepoint.author')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="date"
                         :label="this.$t('tobacco.tstorage.smokepoint.date')"
                         width="100"
                         show-overflow-tooltip />
        ...
        <el-table-column prop="tyear"
                         :label="this.$t('tobacco.tstorage.smokepoint.tyear')"
                         width="100"
                         show-overflow-tooltip />

        <el-table-column prop="ttypeName"
                         :label="this.$t('tobacco.tstorage.smokepoint.ttypeName')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="tstatusName"
                         :label="this.$t('tobacco.tstorage.smokepoint.tstatusName')"
                         width="100"
                         show-overflow-tooltip />

        <el-table-column prop="tvarietyName"
                         :label="this.$t('tobacco.tstorage.smokepoint.tvarietyName')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="trankName"
                         :label="this.$t('tobacco.tstorage.smokepoint.trankName')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="tgradeName"
                         :label="this.$t('tobacco.tstorage.smokepoint.tgradeName')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="standard"
                         :label="this.$t('tobacco.tstorage.smokepoint.standard')">
        ...
        <el-table-column prop="amount"
                         :label="this.$t('tobacco.tstorage.smokepoint.amount')" />
        <el-table-column prop="weight"
                         :label="this.$t('tobacco.tstorage.smokepoint.weight')" />
        <el-table-column prop="control.name"
                         :label="this.$t('tobacco.tstorage.smokepoint.control')" />
        <el-table-column prop="desc"
                         :label="this.$t('tobacco.tstorage.smokepoint.desc')"
                         width="100"
                         show-overflow-tooltip />
...
<script>
const AddForm = () => import("./smokepoint_riseAdd.vue");
const EditForm = () => import("./smokepoint_riseEdit.vue");
...
export default {
 ...
 computed: {
  ...
  methods: {
    ...
    onSearchButtonClick() {
      Promise.all([
        smokepointApi.getAll({
          size: this.formData.pagination.pageSize,
          page: this.formData.pagination.currentPage - 1,
          contains: "annual,serial,title,author,date,operation,countryId,countryName,tyear,ttypeId,ttypeName,tstatusId,tstatusName,tvarietyId,tvarietyName,trankId,trankName,tgradeId,tgradeName,tgradeCode,standard,amount,weight,control,desc,:{keyword}:true".format(
            { keyword: this.formData.pagination.keyword }
          ),
          search: "organization.organizationId:rlike:{orgid};annual:EQ:{annual};serial:EQ:{serial};title:EQ:{title};author:EQ:{author};date:EQ:{date};operation:EQ:{operation};countryId:EQ:{countryId};countryName:EQ:{countryName};tyear:EQ:{tyear};ttypeId:EQ:{ttypeId};ttypeName:EQ:{ttypeName};tstatusId:EQ:{tstatusId};tstatusName:EQ:{tstatusName};tvarietyId:EQ:{tvarietyId};tvarietyName:EQ:{tvarietyName};trankId:EQ:{trankId};trankName:EQ:{trankName};tgradeId:EQ:{tgradeId};tgradeName:EQ:{tgradeName};tgradeCode:EQ:{tgradeCode};standard:EQ:{standard};amount:EQ:{amount};weight:EQ:{weight};control:EQ:{control};type:EQ:{type};".format(
            {
