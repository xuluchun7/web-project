...
-->
      ...
      <el-col :span="8">
        <el-form-item :label="$t('tobacco.tstorage.billOut.organizationName')"
          ...
          <organization-select :value.sync="formItem.destOrganizationId"
                               :label.sync="formItem.destOrganizationName"
    ...
    onSubmitClick(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          Promise.all([
            billOutApi.updateBillOut(this.formItem.id, this.formItem)
          ])
            .then(([response]) => {
              this.formReset(name);
              //重置表单，允许多次操作
