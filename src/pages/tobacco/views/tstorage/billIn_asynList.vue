...
<template>
  <div class='mainPanel'>
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item>
                <el-button type='primary'
                           ...
                           style="width:90px;">原发出库</el-button>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </el-button-group>
    ...
    <div style="border-top: 1px solid gray;color: #606266;font-size: 16px;font-weight: 600;line-height: 32px;margin-top: 1px;">
      <el-row>
      ...
      if (this.formData.selectRow) {
        ...
        Promise.all([billInApi.getItem(this.formData.selectRow.id)])
          .then(([response]) => {
            ...
            if (this.formData.selectRow.sourceBillId) {
              Promise.all([
                billInApi.getItem(this.formData.selectRow.sourceBillId)
              ])
                .then(([response2]) => {
                  this.formData.billOutItemList = response2;
                  this.dataLength =
                    response.length >= response2.length
                      ? response.length
                      : response2.length;
