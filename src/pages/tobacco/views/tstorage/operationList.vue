...
<template>
  <div class='mainPanel'>
    <div class='toolPanel'>
      <div class='queryCriteria'>
        <el-form inline>
          ...
          <el-form-item :label="$t('tobacco.tstorage.operation.balance')"
                        prop="balance">
            <el-select v-model="searchData.balance"
                       style="width: 100%"
                       @change="handleBanceChange"
                       clearable>
      ...
      <div class='toolButton'
        ...
        <el-button-group>
          ...
          <el-button type='primary'
                     @click='addProcessDetail'>收发管理</el-button>
          <el-button type='primary'
    ...
    <main class='contentPanel'>
      <el-table highlight-current-row
              ...
              <el-col :span="12">
                <el-table highlight-current-row
                  ...
                  <el-table-column prop="organization.organizationName"
                                   :label="$t('tobacco.tstorage.operationFromTo.organizationName')"
                                   min-width="150"
                                   show-overflow-tooltip />
                  <el-table-column prop="warehouseName"
                                   :label="$t('tobacco.tstorage.operationFromTo.warehouseName')"
                                   min-width="150"
                                   show-overflow-tooltip />
                  ...
                  <el-table-column prop="threeWarehouseName"
                                   :label="$t('tobacco.tstorage.operationFromTo.threeWarehouseName')"
                                   min-width="150"
                                   show-overflow-tooltip />
                  <el-table-column prop="desc"
                                   :label="$t('tobacco.tstorage.operationFromTo.desc')"
                                   min-width="150"
                                   show-overflow-tooltip />
                  <el-table-column fixed="right"
                    ...
                    <template slot-scope="scope">
                      <el-button type="text"
                                 size="small"
                                 @click='threeDeleteButtonConfirm(props.row,scope.row)'>{{
                        $t('base.buttonDelete')
                        }}
                      </el-button>
                    </template>
        ...
        <el-table-column prop="relationId"
                         :label="$t('tobacco.tstorage.operation.relationId')"
                         width="80"
                         show-overflow-tooltip />
        <!--
        ...
        <el-table-column prop="leaf"
                         :label="$t('tobacco.tstorage.operation.leaf')"
                         width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
            <el-switch v-model="scope.row.leaf"
                       disabled></el-switch>
          </template>
        </el-table-column>
        <el-table-column prop="prefix"
                         :label="$t('tobacco.tstorage.operation.prefix')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="alias"
                         :label="$t('tobacco.tstorage.operation.alias')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="name"
                         :label="$t('tobacco.tstorage.operation.name')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="title"
                         :label="$t('tobacco.tstorage.operation.title')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="balance"
                         :label="$t('tobacco.tstorage.operation.balance')"
                         width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('tobacco.tstorage.operation.immediate')"
                         width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
            <el-switch v-model="scope.row.immediate"
                       disabled />
          </template>
        </el-table-column>
        <el-table-column :label="$t('tobacco.tstorage.operation.checking')"
                         width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
            <el-switch v-model="scope.row.checking"
                       disabled />
          </template>
        </el-table-column>
        <el-table-column label="是否复检"
                         width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
            <el-switch v-model="scope.row.reChecking"
                       disabled />
          </template>
        </el-table-column>
        <el-table-column prop="entry"
                         :label="$t('tobacco.tstorage.operation.entry')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="print"
                         :label="$t('tobacco.tstorage.operation.print')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="visible"
                         :label="$t('tobacco.tstorage.operation.visible')"
                         width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column prop="position"
                         :label="$t('tobacco.tstorage.operation.position')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column prop="constraint"
                         :label="this.$t('tobacco.tstorage.operation.constraint')"
                         width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
            <el-switch v-model="scope.row.constraint"
                       disabled></el-switch>
          </template>
        </el-table-column>
        <el-table-column prop="desc"
                         :label="this.$t('tobacco.tstorage.operation.desc')"
                         width="100"
                         show-overflow-tooltip />
        <el-table-column fixed="right"
                         :label="$t('base.titleOperation')"
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text"
                       size="small"
                       @click="addMenuButtonClick(scope.row,true)">添加菜单
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <template>
      ...
      <el-dialog title="收发管理"
                 :visible.sync="childForm.detailForm"
                 top="10px"
                 :before-close="handleClose">
        <add-detail-form :item.sync=formData.viewSelect
                         :visible.sync="childForm.detailForm" />
      </el-dialog>
      <el-dialog title="添加菜单"
                 ...
                 top="10px"
                 :before-close="handleClose">
        <menu-form :item.sync=formData.viewSelect
                   :visible.sync="childForm.menuForm" />
      </el-dialog>
    ...
    onSearchButtonClick() {
      Promise.all([
        operationApi.getAll({
          ...
          sort: "alias,ASC",
          search: "balance:EQ:{balance}".format({
            balance: this.searchData.balance
          }),
          contains: "lead,leaf,prefix,alias,name,title,balance,immediate,checking,entry,print,visible,position,desc,:{keyword}:true".format(
