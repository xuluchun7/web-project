<template>
  <el-form :model="formItem"
    ...
    <el-form-item :label="$t('tobacco.tstorage.standWeightSet.upperWeight')">
      <el-input-number v-model="formItem.upperWeight"
                       style="width: 100%" />
    </el-form-item>
    <el-form-item :label="$t('tobacco.tstorage.standWeightSet.lowerWeight')">
      <el-input-number v-model="formItem.lowerWeight"
                       style="width: 100%" />
    </el-form-item>
  ...
  methods: {
    ...
    onSubmitClick(name) {
      this.$refs[name].validate(valid => {
        ...
        if (valid) {
          Promise.all([
            standWeightSetApi.updateStandWeightSet(this.item.id, this.formItem)
          ])
            .then(([response]) => {
