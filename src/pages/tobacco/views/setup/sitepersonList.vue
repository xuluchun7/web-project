  ...
  <div class='mainPanel'>
    <div class='toolPanel'>
      ...
      <div class='toolButton' style="min-width: 210px">
        <el-button-group>
          ...
          <el-button type='primary' @click='childForm.addForm=true' >{{ $t('base.buttonAdd') }}
          </el-button>
    ...
    <main class='contentPanel'>
      <el-table
        ...
        <el-table-column prop="webmaster" :label="this.$t('tobacco.setup.siteperson.webmaster')"/>
        <el-table-column prop="clerk" :label="this.$t('tobacco.setup.siteperson.clerk')"/>
        <el-table-column prop="ratingstaff" :label="this.$t('tobacco.setup.siteperson.ratingstaff')"/>
        <el-table-column prop="overweight" :label="this.$t('tobacco.setup.siteperson.overweight')"/>
        <el-table-column prop="packer" :label="this.$t('tobacco.setup.siteperson.packer')"/>
        <el-table-column prop="qualitymanager" :label="this.$t('tobacco.setup.siteperson.qualitymanager')"/>
        <el-table-column prop="precheck" :label="this.$t('tobacco.setup.siteperson.precheck')"/>
        <el-table-column prop="infomanager" :label="this.$t('tobacco.setup.siteperson.infomanager')"/>
        <el-table-column prop="other" :label="this.$t('tobacco.setup.siteperson.other')"/>
        <el-table-column prop="desc" :label="this.$t('tobacco.setup.siteperson.desc')"/>
        <el-table-column
          fixed="right"
...
<script>
  const AddForm = () => import('./sitepersonAdd.vue');
  const EditForm = () => import('./sitepersonEdit.vue');
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([sitepersonApi.deleteSiteperson(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([sitepersonApi.getAll({
          ...
          sort: 'id,ASC',
          contains: 'webmaster,clerk,ratingstaff,overweight,packer,qualitymanager,other,desc:{keyword}:true'.format({keyword: this.formData.pagination.keyword}),
          search: 'organization.organizationId:rlike:{orgid};'.format({
            orgid: this.searchData.organizationId,
          })
        })])
          .then(([response]) => {
            this.formData.sitepersonList = response.content;
