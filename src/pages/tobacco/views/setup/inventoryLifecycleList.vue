  ...
  <div class='mainPanel'>
    ...
    <main class='contentPanel'>
      <el-table
        ...
        :data="formData.inventoryLifecycleList"
        style="width: 100%"
        ...
        <el-table-column prop="classify" :label="this.$t('tobacco.setup.inventoryLifecycle.classify')"/>
        <el-table-column prop="sort" :label="this.$t('tobacco.setup.inventoryLifecycle.sort')"/>
        <el-table-column prop="title" :label="this.$t('tobacco.setup.inventoryLifecycle.title')"/>
        <el-table-column prop="type.name" :label="$t('tobacco.setup.inventoryLifecycle.type')"/>
        <el-table-column
          fixed="right"
...
<script>
  const AddForm = () => import('./inventoryLifecycleAdd.vue');
  const EditForm = () => import('./inventoryLifecycleEdit.vue');
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([inventoryLifecycleApi.deleteInventoryLifecycle(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([inventoryLifecycleApi.getAll({
          ...
          .then(([response]) => {
            this.formData.inventoryLifecycleList = response.content;
