<template>
  <div id="box_pa">
    <div id="select">
      <el-form inline
               style="margin-bottom: 30px;border-bottom: 1px solid #606266;">
    ...
    <div id="nav">
      <el-button-group>
        <el-button type="primary"
                   @click="SendBalance">{{$t('tobacco.setup.balance.tab2')}}</el-button>
        <el-button type="primary"
                   @click="SendUser">{{$t('tobacco.setup.balance.tab3')}}</el-button>
        <el-button type="primary"
                   @click="SendBalanceUser">{{$t('tobacco.setup.balance.tab4')}}</el-button>
        <el-button type="primary"
                   @click="SendUserMapping">{{$t('tobacco.setup.balance.tab5')}}</el-button>
        <el-button type="primary"
                   @click="SendParameter">{{$t('tobacco.setup.balance.tab6')}}</el-button>
        <el-button type="primary"
                   @click="SendPrice">{{$t('tobacco.setup.balance.tab7')}}</el-button>
        <el-button type="primary"
                   @click="SendCard">{{$t('tobacco.setup.balance.tab8')}}</el-button>
        <el-button type="primary"
                   @click="SendContract">{{$t('tobacco.setup.balance.tab9')}}</el-button>
      </el-button-group>
    </div>
    <div class="content"
  ...
  methods: {
    ...
    SendBalance() {
      if (this.checkHost()) {
        ...
        Promise.all([balanceApi.getBalanceJson(this.formItem.organizationId)])
          .then(([response]) => {
    ...
    SendUser() {
      if (this.checkHost()) {
        ...
        this.message = this.$t("base.loading");
        Promise.all([
          balanceApi.getBalanceUserJson(this.formItem.organizationId)
        ])
          .then(([response]) => {
            console.log(response);
            if (response.code === -1) {
    ...
    SendUserMapping(url, data) {
      if (this.checkHost()) {
        this.message = this.$t("base.loading");
        Promise.all([
          balanceAssignAliasApi.getUserMappingJson(this.formItem.organizationId)
        ])
          .then(([response]) => {
            // response = JSON.parse(response);
            console.log(response);
    ...
    SendParameter(url, data) {
      if (this.checkHost()) {
        ...
        Promise.all([
          parameterApi.getParameterJson(year, this.formItem.organizationId)
        ])
          .then(([response]) => {
            // response = JSON.parse(response);
            console.log(response);
    ...
    SendPrice(url, data) {
      if (this.checkHost()) {
        ...
        let year = new Date().getFullYear();
        // year = 2012;
        Promise.all([priceApi.getPriceJson(year, this.formItem.organizationId)])
          .then(([response]) => {
            // response = JSON.parse(response);
            console.log(response);
    ...
    checkHost: function() {
      if (
        this.formItem.host !== "" &&
        this.formItem.host !== undefined &&
        this.formItem.port !== "" &&
        this.formItem.port !== undefined
      ) {
    ...
    iterator: function() {
      ...
      if (that.currentIndex === that.count - 1) {
        (function() {
          console.log(
            that.arr.slice(that.currentIndex * that.size, that.arr.length)
          );
          let sendData = {
            contractList: that.arr.slice(
              that.currentIndex * that.size,
              that.arr.length
            )
          };
          console.log("----------------------------------------");
          ...
          fetch(
            ...
            .then(j => {
              if (j.code === 1) {
                ...
                that.message =
                  "推送中...当前进度" +
                  Math.floor((that.currentIndex / that.count) * 100) +
                  "%";
                console.log(j.message);
                that.iterator(index);
              }
...
#box_pa .el-button:focus,
#box_pa .el-button:hover {
  background: #85ce61;
  border-color: #85ce61;
  color: #fff;
}
