  ...
  <div class='mainPanel'>
    <div class='toolPanel'>
      <div class='queryCriteria'>
        <el-form inline >
          <el-form-item :label="$t('tobacco.setup.lineRole.lineId')">
            <el-select :placeholder="$t('base.pleaseSelect')"
                       v-model=" searchData.lineId" clearable>
              <el-option
    ...
    <main class='contentPanel'>
      <el-table
        ...
        :data="formData.lineRoleList"
        style="width: 100%"
        :row-class-name="tableRowClassName">
        <el-table-column type="index">
        </el-table-column>
        <el-table-column prop="lineId" :label="this.$t('tobacco.setup.lineRole.lineId')"/>
        <el-table-column prop="roleId" :label="this.$t('tobacco.setup.lineRole.roleId')"/>
        <el-table-column prop="name" :label="this.$t('tobacco.setup.lineRole.name')" width="100"
                         show-overflow-tooltip/>
        <el-table-column prop="required" :label="this.$t('tobacco.setup.lineRole.required')">
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.required" disabled>
            </el-switch>
          </template>
        </el-table-column>
        <el-table-column prop="requireMin" :label="this.$t('tobacco.setup.lineRole.requireMin')"/>
        <el-table-column prop="requireMax" :label="this.$t('tobacco.setup.lineRole.requireMax')"/>
        <el-table-column prop="position" :label="this.$t('tobacco.setup.lineRole.position')"/>
        <el-table-column prop="desc" :label="this.$t('tobacco.setup.lineRole.desc')" width="100"
                         show-overflow-tooltip/>
        <el-table-column
          fixed="right"
    ...
    <template>
      <el-dialog :title="$t('base.buttonAdd')" :visible.sync="childForm.addForm" top="10px" :before-close="handleClose">
        <add-form :modelIdList="formData.modelIdList"/>
      </el-dialog>
      <el-dialog :title="$t('base.buttonEdit')" :visible.sync="childForm.editForm" top="10px"
        ...
        <edit-form :item.sync=formData.viewSelect :isEdit=childForm.isEdit :visible.sync="childForm.editForm"
                   :modelIdList="formData.modelIdList" @refresh="onSearchButtonClick"/>
      </el-dialog>
      <el-dialog :title="$t('base.buttonView')" :visible.sync="childForm.viewForm" top="10px"
        ...
        <edit-form :item=formData.viewSelect :isEdit=childForm.isEdit :visible.sync="childForm.viewForm"
                   :modelIdList="formData.modelIdList"
        />
      </el-dialog>
    </template>
  </div>
...
<script>
  const AddForm = () => import('./lineRoleAdd.vue');
  const EditForm = () => import('./lineRoleEdit.vue');
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([lineRoleApi.deleteLineRole(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([lineRoleApi.getAll({
          ...
          sort: 'position,ASC',
          search: 'lineId:EQ:{lineId};(lineId:like:{keyword}:or;roleId:like:{keyword}:or;name:like:{keyword}:or;desc:like:{keyword}:or;)'.format({
            lineId: this.searchData.lineId,
            keyword: this.formData.pagination.keyword
          })
        })])
          .then(([response]) => {
            this.formData.lineRoleList = response.content;
