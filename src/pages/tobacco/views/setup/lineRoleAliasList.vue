      ...
      <div class='queryCriteria'>
        <el-form inline>
          <el-form-item :label="$t('tobacco.setup.lineRoleAlias.name')">
            <el-cascader :options="formData.options"
    ...
    <main class='contentPanel'>
      <el-table highlight-current-row
        ...
        <el-table-column prop="lineId"
                         :label="this.$t('tobacco.setup.lineRoleAlias.lineId')">
          <template slot-scope="scope">
            {{formData.lines[scope.row.lineId]}}
          </template>
        </el-table-column>
        <el-table-column prop="roleId"
                         :label="this.$t('tobacco.setup.lineRoleAlias.roleId')">
          <template slot-scope="scope">
        ...
        <el-table-column prop="code"
                         :label="this.$t('tobacco.setup.lineRoleAlias.code')" />
        <el-table-column prop="name"
                         :label="this.$t('tobacco.setup.lineRoleAlias.name')" />
        <el-table-column prop="desc"
                         :label="this.$t('tobacco.setup.lineRoleAlias.desc')" />
                ...
                arr.forEach(function(val) {
                  val["children"].forEach(function(val2) {
                    data3.forEach(function(val3) {
                      if (
                        val3.lineId === val["value"] &&
                        val3.roleId === val2["value"]
                      ) {
                        tempObj = {};
  ...
  methods: {
    formChange(label, value, values) {
      this.searchData.lineId =
        label[0] === undefined
          ? (this.searchData.lineId = "")
          : (this.searchData.lineId = label[0]);
      this.searchData.roleId =
        label[1] === undefined
          ? (this.searchData.roleId = "")
          : (this.searchData.roleId = label[1]);
      this.searchData.id =
        label[2] === undefined
          ? (this.searchData.id = "")
          : (this.searchData.id = label[2]);
    },
    ...
    deleteButtonClick() {
      ...
      Promise.all([
        lineRoleAliasApi.deleteLineRoleAlias(this.formData.selectRow.id)
      ])
        .then(([response]) => {
          this.$message({
            type: "info",
    ...
    onSearchButtonClick() {
      Promise.all([
        lineRoleAliasApi.getAll({
          ...
          sort: "code,ASC",
          contains: "code,name,desc:{keyword}:true".format({
