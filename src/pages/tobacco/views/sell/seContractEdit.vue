<template>
  <el-form :model="formItem"
      ...
      <el-col :span="8">
        <el-form-item :label="$t('tobacco.tsell.seContract.technicianName')">
          <el-select ref='employeeSelect'
                     ...
                     v-model='formItem.technicianName'
                     filterable
                     remote
                     placeholder="请输入关键词"
    ...
    <el-form-item>
      <el-button :disabled="this.menuProps.buttonDisabled"
                 v-show='isEdit'
                 type="primary"
                 style="float: right "
...
export default {
  ...
  methods: {
    checkNumber() {
      ...
      Promise.all([
        seContractApi.checkNumer(1, this.formItem.number, {
          id: this.formItem.id
        })
      ])
        .then(([response]) => {
    ...
    load() {
      ...
      if (
        ...
        this.formItem.control !== null &&
        this.formItem.control !== undefined
      ) {
        this.formItem.control = this.formItem.control.id;
      }
