  ...
  <div class='mainPanel'>
    ...
    <main class='contentPanel'>
      <el-table highlight-current-row
        ...
        <el-table-column prop="invoiceId"
                         :label="this.$t('tobacco.tsell.invoiceTrace.invoiceId')" />
        <el-table-column prop="timeId"
                         :label="this.$t('tobacco.tsell.invoiceTrace.timeId')" />
        <el-table-column prop="code"
                         :label="this.$t('tobacco.tsell.invoiceTrace.code')" />
        <el-table-column prop="action"
                         :label="this.$t('tobacco.tsell.invoiceTrace.action')" />
        <el-table-column prop="operator"
                         :label="this.$t('tobacco.tsell.invoiceTrace.operator')" />
        <el-table-column prop="message"
                         :label="this.$t('tobacco.tsell.invoiceTrace.message')" />
        <el-table-column prop="loginId"
                         :label="this.$t('tobacco.tsell.invoiceTrace.loginId')" />
        <el-table-column prop="loginName"
                         :label="this.$t('tobacco.tsell.invoiceTrace.loginName')" />
        <el-table-column prop="desc"
                         :label="this.$t('tobacco.tsell.invoiceTrace.desc')" />
...
export default {
  ...
  methods: {
    ...
    deleteButtonClick() {
      ...
      Promise.all([
        invoiceTraceApi.deleteInvoiceTrace(this.formData.selectRow.id)
      ])
        .then(([response]) => {
          this.$message({
            type: "info",
    ...
    onSearchButtonClick() {
      Promise.all([
        invoiceTraceApi.getAll({
          size: this.formData.pagination.pageSize,
          sort: "id,ASC",
          page: this.formData.pagination.currentPage - 1,
          contains: "invoiceId,timeId,code,action,operator,message,loginId,loginName,desc,:{keyword}:true".format(
            { keyword: this.formData.pagination.keyword }
          ),
          search: "invoiceId:EQ:{invoiceId};timeId:EQ:{timeId};code:EQ:{code};action:EQ:{action};operator:EQ:{operator};message:EQ:{message};loginId:EQ:{loginId};loginName:EQ:{loginName};desc:EQ:{desc};".format(
            {
              invoiceId: this.searchData.invoiceId,
              timeId: this.searchData.timeId,
              code: this.searchData.code,
              action: this.searchData.action,
              operator: this.searchData.operator,
              message: this.searchData.message,
              loginId: this.searchData.loginId,
