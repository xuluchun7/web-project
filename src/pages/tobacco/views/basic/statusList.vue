  ...
  <div class='mainPanel'>
    ...
    <main class='contentPanel'>
      <el-table
        ...
        :data="formData.statusList"
        style="width: 100%"
        :row-class-name="tableRowClassName">
        <el-table-column
          type="index">
        </el-table-column>
        <el-table-column prop="id" :label="this.$t('tobacco.basic.status.id')"/>
        <el-table-column prop="lead" :label="this.$t('tobacco.basic.status.lead')"/>
        <el-table-column prop="leaf" :label="this.$t('tobacco.basic.status.leaf')"/>
        <el-table-column prop="name" :label="this.$t('tobacco.basic.status.name')"/>
        <el-table-column prop="visible" :label="this.$t('tobacco.basic.status.visible')"/>
        <el-table-column prop="desc" :label="this.$t('tobacco.basic.status.desc')"/>
...
<script>
  import AddForm from './statusAdd.vue';
  import EditForm from './statusEdit.vue';
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([statusApi.deleteStatus(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([statusApi.getAll({
          ...
          .then(([response]) => {
            this.formData.statusList = response.content;
