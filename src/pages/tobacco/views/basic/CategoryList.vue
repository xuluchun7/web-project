...
<template>
    <div class='mainPanel'>
        ...
        <main class='contentPanel'>
            <el-table highlight-current-row border @current-change="handleCurrentChange" :data="formData.hrClassifyList" style="width: 100%" :row-class-name="tableRowClassName">
                <el-table-column type="index">
                </el-table-column>
                <el-table-column prop="id" :label="this.$t('base.code')" />
                <el-table-column prop="lead" :label="this.$t('tobacco.thuman.hrClassify.lead')" />
                <el-table-column :label="this.$t('tobacco.thuman.hrClassify.leaf')">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.leaf" disabled/>
                    </template>
                </el-table-column>
                <el-table-column prop="name" :label="this.$t('tobacco.thuman.hrClassify.name')" />
                <el-table-column :label="this.$t('tobacco.thuman.hrClassify.visible')">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.visible" disabled/>
                    </template>
                </el-table-column>
                <el-table-column prop="position" :label="this.$t('tobacco.thuman.hrClassify.position')" />
                <el-table-column prop="desc" :label="this.$t('tobacco.thuman.hrClassify.desc')" />
...
<script>
const AddForm = () => import('./CategoryAdd.vue');
const EditForm = () => import('./CategoryEdit.vue');
...
export default {
  ...
  methods: {
    ...
    deleteButtonClick() {
      ...
      Promise.all([hrClassifyApi.deleteHrClassify(this.formData.selectRow.id)])
        .then(([response]) => {
          this.$message({
            type: 'info',
    ...
    onSearchButtonClick() {
      Promise.all([
        hrClassifyApi.getAll({
          size: this.formData.pagination.pageSize,
          sort: 'id,ASC',
          page: this.formData.pagination.currentPage - 1,
          contains: 'id,name,lead:{keyword}:true'.format({keyword: this.formData.pagination.keyword})
        })
      ])
        .then(([response]) => {
          this.formData.hrClassifyList = response.content;
