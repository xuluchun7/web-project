<template>
  <el-form :model="formItem" label-width="80px" :rules="ruleValidate" ref="formValidate">
    <el-form-item :label="$t('tobacco.basic.color.typeId')" prop="typeId">
      <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="formItem.typeId"/>
    </el-form-item>
    <el-form-item :label="$t('tobacco.basic.color.colorId')" prop="colorId">
      <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="formItem.colorId"/>
    </el-form-item>
    <el-form-item :label="$t('tobacco.basic.color.name')" prop="name">
      <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="formItem.name"/>
    </el-form-item>
    <el-form-item :label="$t('tobacco.basic.color.visible')" prop="visible">
      <el-switch v-model="formItem.visible"/>
    </el-form-item>
    <el-form-item :label="$t('tobacco.basic.color.desc')" prop="desc">
    ...
    methods: {
      onSubmitClick(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            Promise.all([colorApi.saveColor(this.formItem)])
              .then(([response]) => {
                this.formReset(name);
                //重置表单，允许多次操作
                this.$message({
