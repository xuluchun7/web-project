<template>
    <el-form :model="formItem" label-width="80px" :rules="ruleValidate" ref="formValidate">
        ...
        <el-form-item :label="$t('tobacco.thuman.hrClassify.lead')" prop="lead">
            <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="formItem.lead" />
        </el-form-item>

        <el-form-item :label="$t('tobacco.thuman.hrClassify.name')" prop="name">
            <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="formItem.name" />
        </el-form-item>

        <el-form-item :label="$t('tobacco.thuman.hrClassify.position')" prop="position">
            <el-input v-bind:placeholder="$t('base.pleaseInput')" v-model="formItem.position" />
        </el-form-item>
         <el-form-item :label="$t('tobacco.thuman.hrClassify.visible')" prop="visible">
            <el-switch v-model="formItem.visible" />
        </el-form-item>
          <el-form-item :label="$t('tobacco.thuman.hrClassify.leaf')" prop="leaf">
            <el-switch v-model="formItem.leaf" />
        </el-form-item>
        <el-form-item :label="$t('tobacco.thuman.hrClassify.desc')" prop="desc">
  ...
  methods: {
    onSubmitClick(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          Promise.all([hrClassifyApi.saveHrClassify(this.formItem)])
            .then(([response]) => {
              this.formReset(name);
              //重置表单，允许多次操作
              this.$message({
