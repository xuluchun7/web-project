  ...
  <div class='mainPanel'>
    ...
    <main class='contentPanel'>
      <el-table
        ...
        :data="formData.colorList"
        style="width: 100%"
        :row-class-name="tableRowClassName">
        <el-table-column
          type="index">
        </el-table-column>
        <el-table-column prop="typeId" :label="this.$t('tobacco.basic.color.typeId')"/>
        <el-table-column prop="colorId" :label="this.$t('tobacco.basic.color.colorId')"/>
        <el-table-column prop="name" :label="this.$t('tobacco.basic.color.name')"/>
        <el-table-column prop="visible" :label="this.$t('tobacco.basic.color.visible')"/>
        <el-table-column prop="desc" :label="this.$t('tobacco.basic.color.desc')"/>
...
<script>
  import AddForm from './colorAdd.vue';
  import EditForm from './colorEdit.vue';
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([colorApi.deleteColor(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([colorApi.getAll({
          ...
          contains: 'id,name,desc:{keyword}:true'.format({keyword: this.formData.pagination.keyword}),
          'search': ''.format({})
        })])
          .then(([response]) => {
