    ...
    methods: {
      ...
      onSubmitClick(name) {
        this.$refs[name].validate((valid) => {//进行前端检验
          if (valid) {
            Promise.all([colorApi.updateColor(this.item.id, this.formItem)])
              .then(([response]) => {
