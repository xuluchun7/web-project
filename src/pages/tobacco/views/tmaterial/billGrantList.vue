        ...
        <el-table highlight-current-row
          ...
          <el-table-column prop="serial"
            ...
            <template slot-scope="scope">
              <el-button type="text"
                         @click="onShowPhoto(scope.row)">{{scope.row.serial}}</el-button>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="arriveOrganizationName"
            ...
            <template slot-scope="scope">
              <el-popover placement="right"
                          width="600"
                          trigger="click">
                <el-collapse v-loading="loadingMore"
                             v-model="activeNames">
                  <el-collapse-item title="档案信息"
                                    name="1">
                    <el-form :model="getFarmer(scope.row,1)"
                             class="cas-group  cas-flex-2 more">
                      <el-form-item :label="$t('tobacco.thuman.hrFarmer.number')">
                        <span>{{getFarmer(scope.row,1).number}}</span>
                      </el-form-item>
                      <el-form-item :label="$t('tobacco.thuman.hrFarmer.identity')">
                        <span>{{getFarmer(scope.row,1).identity}}</span>
                      </el-form-item>
                      <el-form-item :label="$t('tobacco.thuman.hrFarmer.phone')">
                        <span>{{getFarmer(scope.row,1).phone}}</span>
                      </el-form-item>
                      <el-form-item :label="$t('tobacco.thuman.hrFarmer.bankName')">
                        <span>{{getFarmer(scope.row,1).bankName}}</span>
                      </el-form-item>
                      <el-form-item :label="$t('tobacco.thuman.hrFarmer.bankAccount')">
                        <span>{{getFarmer(scope.row,1).bankAccount}}</span>
                      </el-form-item>
                      <el-form-item :label="$t('tobacco.thuman.hrFarmer.address')"
                                    style="width:100%">
                        <span>{{getFarmer(scope.row,1).address}}</span>
                      </el-form-item>
                    </el-form>
                  </el-collapse-item>
                  <el-collapse-item title="种植信息"
                                    name="2">
                    <el-form :model="getFarmer(scope.row,2)"
                             class="cas-group  cas-flex-2 more">
                      <el-form-item :label="$t('tobacco.tfarm.plantPlan.serial')">
                        <span>{{getFarmer(scope.row,2).serial}}</span>
                      </el-form-item>
                      <el-form-item :label="$t('tobacco.tfarm.plantPlan.area')">
                        <span>{{getFarmer(scope.row,2).area}} 亩</span>
                      </el-form-item>
                    </el-form>
                  </el-collapse-item>
                ...
                <el-button slot="reference"
                           type="warning"
                           plain
                           @click="onFarmerClick(scope.row)">{{scope.row.arriveOrganizationName}}</el-button>
              </el-popover>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getFarmer(row, type) {
      if (row.farmerInfo === undefined) {
        return {};
      }
      switch (type) {
        case 1:
          return row.farmerInfo.farmer;
        case 2:
          return row.farmerInfo.plantPlan;
      }
    },
    onFarmerClick(row) {
      //如果烟农信息为空
      if (row.farmerInfo === undefined) {
        ...
        Promise.all([
          hrFarmerApi.getAll({ search: "id:EQ:" + row.arriveOrganizationId }),
          plantPlanApi.getAll({
            search:
              "farmerId:EQ:" +
              row.arriveOrganizationId +
              ";annual:eq" +
          ...
          .then(([response, response2]) => {
            row.farmerInfo = {
              farmer: response.totalElements > 0 ? response.content[0] : {},
              plantPlan: response2.totalElements > 0 ? response2.content[0] : {}
            };
          })
          .catch(error => {})
          .finally(() => {
...
<style scoped>
.more > .el-form-item > .el-form-item__content {
