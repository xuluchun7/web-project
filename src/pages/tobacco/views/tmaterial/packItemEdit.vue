  ...
  <el-form :model="formItem"
    ...
    <el-form-item :label="$t('tobacco.tmaterial.packItem.material')">
      <el-autocomplete class="inline-input"
                       style="width:100%"
                       :value="formItem.price.material.name"
...
export default {
  ...
  methods: {
    ...
    unitSelectChange(value) {
      ...
      this.formItem.price.materialUnit.measureName = name;
    },
    ...
    handleSelect(item) {
      this.formItem.price.material = JSON.parse(JSON.stringify(item));
      this.formData.unitList = [];
      this.formItem.price.materialUnit = {};
      Promise.all([
        materialUnitApi.getAll({
          size: 100,
          search: "material.id:eq:" + this.formItem.price.material.id
        })
      ])
        .then(([unitResponse]) => {
