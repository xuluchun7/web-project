    ...
    <main class='contentPanel'>
      <el-table highlight-current-row
        ...
        <el-table-column prop="lead"
                         :label="this.$t('tobacco.tmisc.scoringRubric.lead')" />
        <el-table-column prop="leaf"
                         :label="this.$t('tobacco.tmisc.scoringRubric.leaf')" />
        <el-table-column prop="code"
                         :label="this.$t('tobacco.tmisc.scoringRubric.code')" />
        <el-table-column prop="name"
                         :label="this.$t('tobacco.tmisc.scoringRubric.name')" />
        <el-table-column prop="score"
                         :label="this.$t('tobacco.tmisc.scoringRubric.score')" />
        <el-table-column prop="weight"
                         :label="this.$t('tobacco.tmisc.scoringRubric.weight')" />
        <el-table-column prop="visible"
                         :label="this.$t('tobacco.tmisc.scoringRubric.visible')" />
        <el-table-column prop="position"
                         :label="this.$t('tobacco.tmisc.scoringRubric.position')" />
        <el-table-column prop="desc"
                         :label="this.$t('tobacco.tmisc.scoringRubric.desc')" />
...
export default {
  ...
  methods: {
    ...
    deleteButtonClick() {
      ...
      Promise.all([
        scoringRubricApi.deleteScoringRubric(this.formData.selectRow.id)
      ])
        .then(([response]) => {
          this.$message({
            type: "info",
    ...
    onSearchButtonClick() {
      Promise.all([
        scoringRubricApi.getAll({
          size: this.formData.pagination.pageSize,
          page: this.formData.pagination.currentPage - 1,
          contains: "lead,leaf,code,name,score,weight,visible,position,desc,:{keyword}:true".format(
            { keyword: this.formData.pagination.keyword }
          ),
          search: "lead:EQ:{lead};leaf:EQ:{leaf};code:EQ:{code};name:EQ:{name};score:EQ:{score};weight:EQ:{weight};visible:EQ:{visible};position:EQ:{position};desc:EQ:{desc};".format(
            {
