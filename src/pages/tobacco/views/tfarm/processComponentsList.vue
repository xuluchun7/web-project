          ...
          <template slot-scope="props">
            <el-form label-position="left"
              ...
              <el-form-item :label="$t('tobacco.tfarm.processAttr.show')">
                <span>{{ props.row.show|boolean }}</span>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
...
<script>
const AddForm = () => import("./processComponentsAdd.vue");
...
export default {
  ...
  methods: {
    ...
    deleteButtonClick() {
      ...
      Promise.all([processComponentsApi.delete(this.formData.selectRow.id)])
        .then(([response]) => {
          this.$message({
            type: "info",
    ...
    onSearchButtonClick() {
      ...
      if (
        ...
        this.searchData.classify !== undefined
      ) {
        search = "classify:eq:" + this.searchData.classify;
      }
      Promise.all([
        processComponentsApi.getAll({
          ...
          sort: "code",
          contains: "code,label:{keyword}:true".format({
