<template>
  <el-form :model="formItem"
    ...
    <el-row>
      ...
      <el-col :span="12">
        <el-form-item :label="$t('tobacco.thuman.hrTeam.finish')"
                      prop='date'>
          <el-date-picker v-model="formItem.finish"
                          type="date"
                          style="width:100%;"
                          :placeholder="$t('base.pleaseSelect')">
          </el-date-picker>
    ...
    <el-form-item>
      <el-button v-show='isEdit'
                 type="primary"
                 style="float: right"
                 @click="submitForm('formItem')">{{ $t('base.buttonSave') }} </el-button>
    </el-form-item>
  </el-form>
</template>
<script>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          Promise.all([hrTeamApi.updateHrTeam(this.item.id, this.formItem)])
            .then(([response]) => {
              this.$message({
