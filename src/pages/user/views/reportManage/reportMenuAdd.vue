  ...
  <el-form :model="formItem"
    ...
    <el-form-item :label="$t('menu.url')"
      ...
      <el-input v-model="formItem.url"
                :placeholder="$t('base.pleaseInput')">
        <template slot="append">{{'?serial=' + formItem.serial}}</template>
      </el-input>
    </el-form-item>
...
export default {
  ...
  created() {
    Promise.all([systemApi.getAll()])
      ...
      .catch(error => {});
    this.formItem.path = "/reportManage/showReportList";
    this.formItem.serial = this.item.serial;
    console.info(this.item.serial);
  },
  methods: {
    onSubmitClick(name) {
      if (
      ...
      this.formItem.url = this.formItem.url + "?serial=" + this.formItem.serial;
      console.info(this.formItem.url);
