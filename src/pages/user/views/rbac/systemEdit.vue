  ...
  export default {
    ...
    data() {
      return {
        ...
        ruleValidate: {
          code: [
            {required: true, message: '编码至少3个字符', trigger: 'blur', min: 3, max: 32}
          ],
          name: [
            {required: true, message: '登陆名至少2个字符', trigger: 'blur', min: 2}
          ],
          title: [
            {required: true, message: '登陆名至少2个字符', trigger: 'blur', min: 2}
          ],
    ...
    methods: {
      ...
      onSubmitClick(name) {
        this.$refs[name].validate((valid) => { // 进行前端检验
          if (valid) {
            Promise.all([systemApi.updateSystem(this.item.id, this.formItem)])
              .then(([response]) => {
                this.formReset(name);
                this.$message({
