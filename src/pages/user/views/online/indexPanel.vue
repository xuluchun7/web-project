...
<template>
  <div style="display: inherit">
    <div style="text-align: right">
      <el-button plain
                 type="primary"
                 @click="creatProperty()">增加</el-button>
    </div>
    <el-table row-key="index"
      ...
      <el-table-column prop="indexName"
                       :label="$t('onlineForm.indexName')">
        <template slot-scope="scope">
          <el-input v-show="scope.row._edit"
                    v-model="scope.row.indexName"
                    clearable></el-input>
          <span v-show="!scope.row._edit"> {{ scope.row.indexName}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="indexColumns"
                       :label="$t('onlineForm.indexColumns')">
        <template slot-scope="scope">
          <el-input v-show="scope.row._edit"
      ...
      <el-table-column prop="indexType"
                       :label="$t('onlineForm.indexType')">
        <template slot-scope="scope">
          <el-switch :disable="!scope.row._edit"
                     v-model="scope.row.indexType"></el-switch>
        </template>
      </el-table-column>
