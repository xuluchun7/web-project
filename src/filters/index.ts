...
export function timeAgo(time: any) {
  const between = Date.now() / 1000 - Number(time, 2);
  if (between < 3600) {
...
export function toThousandslsFilter(num: any) {
  return (+num || 0)
    .toString()
    .replace(/^-?\d+/g, m => m.replace(/(?=(?!\b)(\d{3})+$)/g, ','));
}

export function parseDate(date: any, pattern: any) {
  return moment(date).format(pattern);
}
export function Currency(value: any) {
  return Vue.prototype.$accounting.formatMoney(value, '¥');
}
export function Number(value: any, length: number) {
  return Vue.prototype.$accounting.formatNumber(value, length);
}
export function Percent(value: number, length: number) {

  return (
    Vue.prototype.$accounting.formatNumber(value * 100, length - 2) + '%'
  );
}
